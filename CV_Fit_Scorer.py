...
st.caption("CV vs Job Description Fit Scorer")
    ...
    if not cv_text:
        ...
        try:
            with st.spinner("Analysing fit with Claude..."):
                analysis = analyze_fit(cv_text, jd_text)
            # Stash snapshot so the result + save form survive widget reruns.
            st.session_state.analysis_result = analysis
            st.session_state.analysis_jd = jd_text
            st.session_state.analysis_cv_name = st.session_state.active_cv_name
            st.session_state.saved_app_id = None
            # Clear any leftover save-form inputs from a previous analysis.
            for k in ("save_company", "save_role"):
...
if result:
    ...
    if st.session_state.saved_app_id is not None:
        st.success(
            f"Saved to tracker as application #{st.session_state.saved_app_id}. "
            "Open the **📋 Application Tracker** page in the sidebar to manage it."
        )
    else:
        save_col1, save_col2 = st.columns(2)
        with save_col1:
            company_input = st.text_input(
                "Company", key="save_company", placeholder="e.g. Anthropic"
            )
        with save_col2:
            role_input = st.text_input(
                "Role", key="save_role", placeholder="e.g. ML Engineer"
            )

        can_save = bool(company_input.strip() and role_input.strip())
        if st.button("💾 Save to tracker", disabled=not can_save, type="secondary"):
            app_id = database.insert_application(
                company=company_input.strip(),
                role=role_input.strip(),
                cv_name=st.session_state.analysis_cv_name,
                jd_text=st.session_state.analysis_jd,
